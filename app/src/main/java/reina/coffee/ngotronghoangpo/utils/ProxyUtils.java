    ...
    public boolean isProxyReady(@NonNull Activity activity) {
        if (mPreferences.getProxyChoice() == Constants.PROXY_I2P) {
            if (!mI2PHelper.isI2PAndroidRunning()) {
                reina.coffee.ngotronghoangpo.utils.Utils.showSnackbar(activity, R.string.i2p_not_running);
                return false;
            } else if (!mI2PHelper.areTunnelsActive()) {
                reina.coffee.ngotronghoangpo.utils.Utils.showSnackbar(activity, R.string.i2p_tunnels_not_ready);
                return false;
            }
        }
