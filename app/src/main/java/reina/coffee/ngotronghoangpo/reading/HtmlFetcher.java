...
public class HtmlFetcher {
    ...
    static {
        reina.coffee.ngotronghoangpo.reading.SHelper.enableCookieMgmt();
        reina.coffee.ngotronghoangpo.reading.SHelper.enableUserAgentOverwrite();
        reina.coffee.ngotronghoangpo.reading.SHelper.enableAnySSL();
    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        try {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                String domainStr = reina.coffee.ngotronghoangpo.reading.SHelper.extractDomain(url, true);
                String counterStr = "";
    ...
    private reina.coffee.ngotronghoangpo.reading.SCache cache;
    ...
    @NonNull
    public HtmlFetcher setCache(reina.coffee.ngotronghoangpo.reading.SCache cache) {
        this.cache = cache;
        return this;
    }
    ...
    @NonNull
    public reina.coffee.ngotronghoangpo.reading.JResult fetchAndExtract(String url, int timeout, boolean resolve) throws Exception {
    ...
    private reina.coffee.ngotronghoangpo.reading.JResult fetchAndExtract(String url, int timeout, boolean resolve,
                                                                         int maxContentSize, boolean forceReload) throws Exception {
        String originalUrl = url;
        url = reina.coffee.ngotronghoangpo.reading.SHelper.removeHashbang(url);
        String gUrl = reina.coffee.ngotronghoangpo.reading.SHelper.getUrlFromUglyGoogleRedirect(url);
        if (gUrl != null)
            url = gUrl;
        else {
            gUrl = reina.coffee.ngotronghoangpo.reading.SHelper.getUrlFromUglyFacebookRedirect(url);
            if (gUrl != null)
                url = gUrl;
        }
        ...
        if (resolve) {
            ...
            reina.coffee.ngotronghoangpo.reading.JResult res = getFromCache(url, originalUrl);
            if (res != null)
                return res;
            ...
            if (resUrl.isEmpty()) {
                ...
                reina.coffee.ngotronghoangpo.reading.JResult result = new reina.coffee.ngotronghoangpo.reading.JResult();
                if (cache != null)
            ...
            if (!resUrl.equals(url)) {
                ...
                url = reina.coffee.ngotronghoangpo.reading.SHelper.useDomainOfFirstArg4Second(url, resUrl);
            }
        }
        ...
        if (reina.coffee.ngotronghoangpo.reading.SHelper.isDoc(lowerUrl) || reina.coffee.ngotronghoangpo.reading.SHelper.isApp(lowerUrl) || reina.coffee.ngotronghoangpo.reading.SHelper.isPackage(lowerUrl)) {
            // skip
        } else if (reina.coffee.ngotronghoangpo.reading.SHelper.isVideo(lowerUrl) || reina.coffee.ngotronghoangpo.reading.SHelper.isAudio(lowerUrl)) {
            result.setVideoUrl(url);
        } else if (reina.coffee.ngotronghoangpo.reading.SHelper.isImage(lowerUrl)) {
            ...
            if (result.getFaviconUrl().isEmpty())
                result.setFaviconUrl(reina.coffee.ngotronghoangpo.reading.SHelper.getDefaultFavicon(url));
    ...
    private static String fixUrl(String url, String urlOrPath) {
        return reina.coffee.ngotronghoangpo.reading.SHelper.useDomainOfFirstArg4Second(url, urlOrPath);
    }
    ...
    private String fetchAsString(String urlAsString, int timeout, boolean includeSomeGooseOptions)
        ...
        String enc = reina.coffee.ngotronghoangpo.reading.Converter.extractEncoding(hConn.getContentType());
        return createConverter(urlAsString).streamToString(is, enc);
    }

    @NonNull
    private static reina.coffee.ngotronghoangpo.reading.Converter createConverter(String url) {
    ...
    private String getResolvedUrl(@NonNull String urlAsString, int timeout,
        ...
        try {
            ...
            if (responseCode / 100 == 3 && newUrl != null && num_redirects < 5) {
                ...
                newUrl = reina.coffee.ngotronghoangpo.reading.SHelper.useDomainOfFirstArg4Second(urlAsString, newUrl);
    ...
    private reina.coffee.ngotronghoangpo.reading.JResult getFromCache(String url, String originalUrl) {
        if (cache != null) {
