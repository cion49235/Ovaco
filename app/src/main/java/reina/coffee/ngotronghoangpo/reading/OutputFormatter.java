...
public class OutputFormatter {
    ...
    public String getFormattedText(@NonNull Element topNode) {
        ...
        String str = reina.coffee.ngotronghoangpo.reading.SHelper.innerTrim(sb.toString());
    ...
    private int append(@NonNull Element node, @NonNull StringBuilder sb, @NonNull String tagName) {
        ...
        for (Element e : node.select(tagName)) {
            ...
            if (text.isEmpty() || text.length() < getMinParagraph(paragraphWithTextIndex)
                    || text.length() > reina.coffee.ngotronghoangpo.reading.SHelper.countLetters(text) * 2) {
                continue;
            }
